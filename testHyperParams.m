...
for i = 1:length(hypFiles)
    ...
    try
        ...
        %         fprintf('Error occured. Moving to next hyper parameters.\n\n');
        continue;
    end
    ...
    % fprintf('[nMSE] Predictions and Observations:\n');
    % disp(nMSE_Pred)
    ...
    ratio = nMSE_RBD./nMSE_Pred;
    %     fprintf( strcat('[',hypFiles(1).name,'] Order of improvement in prediction:\n' ) );
    %     disp( ratio );
...
%% Displaying Results
max_idx = find( sum(ratio_hist(:,1:6) > 1 ,2) >= 4);
if isempty(max_idx)
...
if size(max_ratio,1) >= 10
    ...
    if isempty(abv_mean_idx)
        ...
        for i=1:length(abv_mean_idx)
            jdx = abv_mean_idx(i);
            fprintf('%8.3f  %8.3f  %8.3f  %8.3f  %8.3f  %8.3f  [%3d]  |   %s\n', ...
                max_ratio_copy(jdx, 1:end), hypFiles(max_ratio_copy(jdx, end)).name );
        end
    end
else
    for i=1:length(max_idx)
